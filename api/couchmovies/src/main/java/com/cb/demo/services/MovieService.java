...
import com.cb.demo.entities.vo.*;
import com.cb.demo.repositories.MovieRepository;
import com.cb.demo.library.EntityExtractor;
import com.cb.demo.library.MovieQueryParser;
...
import static com.cb.demo.library.EntityExtractor.EntityType.PERSON;
import static com.cb.demo.library.EntityExtractor.EntityType.GENRES;
...
public class MovieService implements SearchService {
    ...
    private Result search(String words, Map<String, List<String>> facets, Boolean fuzzy) {
        ...
        ConjunctionQuery conjunctionQuery = SearchQuery.conjuncts(ftsQuery, boostReleaseYearQuery(), boostRuntime(),
                boostPromoted(), boostWeightedRating(), boostPopularity());
    ...
    private AbstractFtsQuery getActorsDisjunctionAdjusted(String words, EntityExtractor entityExtractor,
            boolean fuzzy) {
    ...
    private DisjunctionQuery boostPopularity() {
        ...
        DisjunctionQuery popularityDisjunction = SearchQuery.disjuncts(rangeQuery, rangeQuery2, rangeQuery3,
                rangeQuery4, rangeQuery5);
    ...
    private DisjunctionQuery boostReleaseYearQuery() {
        ...
        DisjunctionQuery yearDisjunction = SearchQuery.disjuncts(rangeQuery, penalizationQuery, penalization1Query,
                penalization2Query, penalization3Query);
    ...
    private DisjunctionQuery boostWeightedRating() {
        ...
        DisjunctionQuery runtimeDisjunction = SearchQuery.disjuncts(weightedRating1, weightedRating2, weightedRating3,
                weightedRating4);
    ...
    private Result getSearchResults(SearchQueryResult result){
        ...
        if (result != null && result.errors().isEmpty()) {
            ...
            while (resultIterator.hasNext()) {
                ...
                //movies.add( new SearchResult(movieRepository.findOne(row.id()), new QueryStats(counter, row)));
                counter++;
            }
